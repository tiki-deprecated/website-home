...
module.exports = {
  ...
  addContact: function(props, callback){
    dynamodb.putItem(
      {
  ...
  findContacts: function(contact, callback){
    dynamodb.query(
      {
        TableName: process.env.DYNAMODB_TABLE,
        ...
        KeyConditionExpression: 'contact_info = :contact and timestamp_utc > :time'
      },
      function (err, data) {
        if (err) {
          console.log(err)
  ...
  updateContactOpt: function(contact, timestamp, optIn, callback){
    dynamodb.updateItem(
      {
        TableName: process.env.DYNAMODB_TABLE,
        Key: {
